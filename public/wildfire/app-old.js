...
function startCountdown(updatedAt, nextUpdate) {
  ...
  if (msLeft <= 0) {
    // Data is past its next_update time — show age, schedule a single retry
    document.getElementById('next-update').textContent = '';
    refreshTimer = setTimeout(() => loadData(), 15 * 60 * 1000);
    // Keep the "Updated Xd ago" label fresh
    countdownInterval = setInterval(() => {
      document.getElementById('last-updated').textContent = `Updated ${formatAge(updatedAt)}`;
    }, 60000);
  } else {
    countdownInterval = setInterval(() => {
      const remaining = new Date(nextUpdate) - Date.now();
      if (remaining <= 0) {
